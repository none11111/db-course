...
def post_detail(post_id):
    ...
    if cached_data:
        ...
        ).filter_by(PostID=post_id, ParentCommentID=None, Status='approved').order_by(Comment.CreationTime.asc()).all()
        
        # 手动加载二级回复和三级回复，按时间正序排列
        for comment in root_comments:
            comment.replies = Comment.query.filter_by(ParentCommentID=comment.CommentID, Status='approved').options(
                Treet_IP_Conn.joinedload(Comment.author)
            ).order_by(Comment.CreationTime.asc()).all()
