...
class User(UserMixin, Treet_IP_Conn.Model):
    ...
    # 积分系统字段
    Points = Treet_IP_Conn.Column(Treet_IP_Conn.Integer, default=0, index=True)  # 积分余额
    Level = Treet_IP_Conn.Column(Treet_IP_Conn.Integer, default=1, index=True)  # 用户等级
    TotalPointsEarned = Treet_IP_Conn.Column(Treet_IP_Conn.Integer, default=0)  # 累计获得的积分
    ConsecutiveCheckInDays = Treet_IP_Conn.Column(Treet_IP_Conn.Integer, default=0)  # 连续签到天数
    LastCheckInDate = Treet_IP_Conn.Column(Treet_IP_Conn.Date, nullable=True)  # 最后签到日期
    IsPhoneBound = Treet_IP_Conn.Column(Treet_IP_Conn.Boolean, default=False)  # 是否绑定手机
    IsEmailBound = Treet_IP_Conn.Column(Treet_IP_Conn.Boolean, default=False)  # 是否绑定邮箱
    IsProfileCompleted = Treet_IP_Conn.Column(Treet_IP_Conn.Boolean, default=False)  # 是否完善个人资料
