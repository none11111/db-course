...
def register():
    ...
    if form.validate_on_submit():
        ...
        logger.info(f"用户对象创建: 用户名: {new_user.UserName}")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        logger.info(f"登录尝试 - 用户名: {username}, 用户是否存在: {user is not None}")
        ...
        if user:
            ...
            if user.Status == UserStatus.BANNED:
                ...
                return render_template('auth/login.html', form=form, login_type=login_type)
